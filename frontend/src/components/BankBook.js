...
function BankBook() {
  ...
  const loadTransactions = async () => {
    ...
    try {
      ...
      txns.push({
        ...
        debit: bankAccount?.opening_balance_type === 'Dr' ? Math.abs(runningBalance) : 0,
        credit: bankAccount?.opening_balance_type === 'Cr' ? Math.abs(runningBalance) : 0,
        balance: runningBalance,
        balance_type: bankAccount?.opening_balance_type || 'Dr'
      });
  ...
  const handleExport = () => {
    ...
    let csv = `Bank Book - ${account?.name || 'Bank Account'}\n`;
    csv += `Period: ${fromDate} to ${toDate}\n\n`;
    csv += 'Date,Particulars,Vch Type,Vch No.,Debit (Deposit),Credit (Withdrawal),Balance\n';
    ...
    a.href = url;
    a.download = `bank-book-${account?.name || 'all'}-${fromDate}-to-${toDate}.csv`;
    a.click();
  ...
  return (
    <div className="page-content">
      <div className="tally-report" id="bank-book-report">
        <div className="tally-report-header">
          {companyName && <p className="tally-company-name">{companyName}</p>}
          <p className="tally-report-title">Bank Book</p>
          {bankAccounts.find(a => a.id === parseInt(selectedAccount)) && (
            <p className="tally-report-subtitle">
              {bankAccounts.find(a => a.id === parseInt(selectedAccount))?.name}
            </p>
          )}
        ...
        {loading ? (
          <div className="tally-loading">Loading Bank Book...</div>
        ) : bankAccounts.length === 0 ? (
          <div className="tally-empty-state">
            <p>No bank accounts found. Create a bank account in Ledger Master.</p>
          </div>
        ) : !selectedAccount ? (
          <div className="tally-empty-state">
            <p>Select a Bank Account to view its transactions</p>
          </div>
        ) : transactions.length <= 1 ? (
          <div className="tally-empty-state">
            <p>No bank transactions found for the selected period</p>
          </div>
        ) : (

...
function DebitNote() {
  ...
  const loadData = async () => {
    ...
    try {
      const [vouchersRes, accountsRes, fyRes] = await Promise.all([
        voucherAPI.getAll({ voucher_type: 'debit_note' }),
  ...
  const removeEntry = (index) => {
    if (currentVoucher.entries.length <= 2) {
      setError('Debit note must have at least 2 lines');
      return;
    }
    setCurrentVoucher(prev => ({
      ...prev,
  ...
  const handleSaveVoucher = async () => {
    ...
    try {
      const voucherData = {
        ...
        narration: currentVoucher.narration || 'Debit Note',
      ...
      const errorMsg = err.response?.data?.error || err.response?.data?.detail || 'Failed to save debit note';
      setError(errorMsg);
      showError(errorMsg);
    } finally {
      setLoading(false);
  ...
  const handleDeleteVoucher = async (id) => {
    if (window.confirm('Are you sure you want to delete this debit note?')) {
      try {
        ...
        showError(err.response?.data?.error || 'Failed to delete debit note');
      }
    }
  };
  ...
  const filteredVouchers = debitNotes
    .filter(voucher => {
  ...
  const stats = useMemo(() => {
    const totalDebitAmount = debitNotes.reduce((sum, dn) => sum + getAmount(dn.entries), 0);
        ...
        <div className="content-card" style={{ marginBottom: '24px' }}>
          <div className="form-section">
            <h3 className="form-section-title">
              {currentVoucher.id ? 'Edit Debit Note' : 'New Debit Note'}
            </h3>
            ...
            <div className="form-grid">
              ...
              <div className="form-field full-width">
                ...
                <textarea
                  ...
                  placeholder="Reason for debit note (e.g., Purchase return, price difference)..."
                ></textarea>
              </div>
            </div>

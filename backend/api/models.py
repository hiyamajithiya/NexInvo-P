...
class InvoiceItem(models.Model):
    ...
    quantity = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, help_text='Quantity of items/units')
    rate = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True, help_text='Rate per unit')
...
class InvoiceFormatSettings(models.Model):
    ...
    show_quantity_column = models.BooleanField(default=False, help_text='Show quantity column in invoice items')
    show_rate_column = models.BooleanField(default=False, help_text='Show rate/unit price column in invoice items')
...
class PasswordResetOTP(models.Model):
    ...
    otp_code = models.CharField(max_length=6)
    is_verified = models.BooleanField(default=False)
    is_used = models.BooleanField(default=False, help_text='Whether password has been reset')
    attempts = models.IntegerField(default=0, help_text='Number of verification attempts')
    ...
    def __str__(self):
        return f"Password Reset OTP for {self.email} - {'Used' if self.is_used else 'Verified' if self.is_verified else 'Pending'}"
    ...
    def generate_otp(cls, email):
        ...
        cls.objects.filter(email=email.lower(), is_used=False).delete()
    ...
    def verify_otp(cls, email, otp_code):
        ...
        try:
            ...
            return False, "No OTP found for this email. Please request a new one.", None
